...
/*------------ Variables for calculation of the flux at the wall -------------*/
	real Jv_wall;
	real Permeability_factor =  1.1467e-11;
	real osmotic_pressure = 2.1e5;
	real rejection = 0.97;
	real operating_pressure = 4.8e5;
	real NV_VEC(psi_vec), NV_VEC(FluxArea), flux;

/*------------ Variables for calculation of the solute at the wall-------------*/
	real A[ND_ND];
...
/*------------ Calculation of the solute concentration at the wall -------------*/
	begin_f_loop(facet, thread_pointer)
	{
...
/*------------ Calculation of the flux at the wall using m_Aw -----------------*/

	// Get the cell values from the boundary thread 
	c0 = F_C0(facet, thread_pointer);
	t0 = F_C0_THREAD(facet, thread_pointer);
	F_AREA(FluxArea, facet, thread_pointer);

	/*If the face lies at the domain boundary, will have to use the face values ...
	else would have to use the values in domain and use it's average value     */

	Jv_wall = Permeability_factor*(operating_pressure - osmotic_pressure*(m_Aw*rejection));
	
	if(BOUNDARY_FACE_THREAD_P(thread_pointer))
	{
		real dens;

		/*Depending on the boundary condition, density may not be defined on the boundary face thread*/
		if(NNULLP(THREAD_STORAGE(thread_pointer, SV_DENSITY)))
			/*Density is set to the face value*/
			dens = F_R(facet,thread_pointer);
		else
		...
		flux = Jv_wall *NV_MAG(FluxArea);

		thread_loop_c(thread_pointer, domain_pointer)
		{
			begin_c_loop(cellt, thread_pointer)
			{
				C_UDMI(cellt, thread_pointer, wall_flux) = Jv_wall * NV_MAG(FluxArea)/2.0;
			}
			end_c_loop(cellt, thread_pointer)
		}
	}
	...
	{
		c1 = F_C1(facet,thread_pointer);
		t1 = F_C1_THREAD(facet, thread_pointer);

		Jv_wall *= (C_R(c0,t0)+ C_R(c1,t1));
		
		//flux = NV_DOT(Jv_wall, A)/2.0; /*Average flux through the face*/
		flux = Jv_wall * NV_MAG(FluxArea)/2.0;
		thread_loop_c(thread_pointer, domain_pointer)
		{
			begin_c_loop(cellt, thread_pointer)
			{
